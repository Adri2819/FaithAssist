  ...
  <main class="relative min-h-screen overflow-hidden bg-slate-100 px-4 py-10 transition-colors duration-300 dark:bg-slate-950">
    ...
    <section class="relative mx-auto w-full max-w-xl rounded-3xl border border-slate-200 bg-white/95 p-7 shadow-2xl backdrop-blur dark:border-white/15 dark:bg-slate-900/85 sm:p-8">
      <div class="mb-6 flex items-center justify-between">
        ...
        <Link href="/login" class="text-sm font-semibold text-slate-600 transition hover:text-slate-900 dark:text-slate-300 dark:hover:text-white">Volver al login</Link>
      </div>

      <div class="mb-6">
        <div class="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
          <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900">1</span>
          Paso 1 de 4
        </div>
        <h1 class="text-2xl font-black tracking-tight text-slate-900 dark:text-slate-100">Recuperar contraseña</h1>
        <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">Ingresa el correo electrónico asociado a tu cuenta para comenzar la verificación.</p>
      </div>
      ...
      <form class="space-y-4" @submit.prevent="submit">
        <div>
          <label class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">Correo electrónico</label>
          <input
            v-model="form.email"
            type="email"
            class="input input-bordered h-12 w-full rounded-xl border-slate-300 bg-white text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
